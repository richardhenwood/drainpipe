		...
		for( int patIdx = 0; patIdx < numPatterns; patIdx++ ) {
			...
			for( int patDataIdx = 0; patDataIdx < pattern.data.length; patDataIdx += 1 ) {
				...
				if( period > 112 ) {
					...
					pattern.data[ patDataIdx ][0] = ( byte ) ( oct * 12 + key );
				}
		...
		for( int patIdx = 0; patIdx < numPatterns; patIdx++ ) {
			...
			while( rowIdx < 64 ) {
				...
				if( chanIdx >= 0 ) {
					...
					pattern.data[ noteOffset ][ 0 ] = ( byte ) noteKey;
